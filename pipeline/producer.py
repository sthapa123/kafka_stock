...
def get_historical_data(symbol=symbol_list[0], outputsize='full'):
    ...
    # get data using Alpha Vantage's API

    url = "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol={}&outputsize={}&interval=1min&apikey={}".format(
        symbol, outputsize, config['api_key'])
...
def get_intraday_data(symbol=symbol_list[0], outputsize='compact', freq='1min'):
    ...
    # get data using Alpha Vantage's API
    url = "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol={}&outputsize={}&interval={" \
          "}&apikey={}".format(symbol, outputsize, freq, config['api_key'])
    req = requests.get(url)
    ...
    # if request failed, return a fake data point
        ...
        time_zone = timeZone
        print('  Failed: Cannot get {}\'s data at {}:{} '.format(symbol, datetime.datetime.now(timezone(time_zone)),
                                                                 req.status_code))
        value = {"symbol": 'None',
                 "time": 'None',
                 "open": 0.,
...
def get_tick_intraday_data(symbol=symbol_list[0]):
    # get data using Alpha Vantage's API
    time_zone = timeZone
    url = "https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={}&apikey={}".format(symbol,
                                                                                               config['api_key2'])
    req = requests.get(url)
...
if __name__ == "__main__":
    ...
    if datetime.datetime.now(timezone(timeZone)).time() > datetime.time(16, 0, 0) or datetime.datetime.now(
            timezone(timeZone)).time() < datetime.time(9, 30, 0):
