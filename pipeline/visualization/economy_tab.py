...
from bokeh.models import PreText, LogColorMapper, Div, ColumnDataSource, HoverTool, Select, HoverTool, LinearAxis, \
    LabelSet, Range1d, PreText, Div
...
def stream_news():
    ...
    def make_dataset(date='2019-08-01'):
        ...
        df.publishedat = pd.DatetimeIndex(pd.to_datetime(df.publishedat, unit='ms')).tz_localize('GMT').tz_convert(
            'US/Pacific').to_pydatetime()
...
def economy_plot():
    ...
    def make_plot(name):
        ...
        hover = HoverTool(tooltips=[("date", "@DATE{%F}"), ('data', "@DATA")], formatters={'DATE': 'datetime'},
                          mode='vline')
    ...
    def callback3(attr, old, new):
        ...
        ['_'.join(i.split()) + '.csv' for i in [indicator1, indicator2, indicator3, indicator4]])
        df1, df2, df3, df4 = read_economy(name1), read_economy(name2), read_economy(name3), read_economy(name4)
        source1.data, source2.data, source3.data, source4.data = df1.to_dict('list'), df2.to_dict('list'), df3.to_dict(
            'list'), df4.to_dict('list')

        e1.title.text, e2.title.text, e3.title.text, e4.title.text = indicator1, indicator2, indicator3, indicator4
        e1.y_range.start, e2.y_range.start, e3.y_range.start, e4.y_range.start = min(source1.data['DATA']) / 1.05, min(
            source2.data['DATA']) / 1.05, min(source3.data['DATA']) / 1.05, min(source4.data['DATA']) / 1.05
        e1.y_range.end, e2.y_range.end, e3.y_range.end, e4.y_range.end = max(source1.data['DATA']) * 1.05, max(
            source2.data['DATA']) * 1.05, max(source3.data['DATA']) * 1.05, max(source4.data['DATA']) * 1.05
