...
def read_company(symbol):
    ...
    buzzsummary = '\n'.join(
        splitTextToTriplet('.'.join(company['summaryProfile']['longBusinessSummary'].split('.')[:3]), 8))
    ...
    for institution in institutionOwnership:
        institution_list.append(
            [institution['organization'], institution['position']['raw'], institution['pctHeld']['fmt']])
...
def stream_price():
    ...
    trans_time = pd.DatetimeIndex(pd.to_datetime(df.time, unit='ms')).tz_localize('GMT').tz_convert(
