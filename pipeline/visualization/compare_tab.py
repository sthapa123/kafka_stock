...
def compare_plot():
    ...
    def make_dataset(start='2014-01-01'):
        ...
        for comp in symbol_list:
            ...
            df_comp['ma5'] = _ma(df_comp.close, 5)
            df_comp['ma10'] = _ma(df_comp.close, 10)
            df_comp['ma30'] = _ma(df_comp.close, 30)
    ...
    p.line('time', 'ma5', line_width=1, color=palte[0], alpha=0.8, muted_color='navy', muted_alpha=0.1, legend='MA5',
           source=source)
    p.line('time', 'ma10', line_width=1, color=palte[1], alpha=0.8, muted_color='navy', muted_alpha=0.1, legend='MA10',
           source=source)
    p.line('time', 'ma30', line_width=1, color=palte[2], alpha=0.8, muted_color='navy', muted_alpha=0.1, legend='MA30',
           source=source)
    ...
    p.vbar('time', width=3, top='volume', color=choice(all_palettes['Set2'][8]), alpha=0.5, y_range_name="volumes",
           source=source)
    ...
    def callback(attr, old, new):
        ...
        p2.y_range.start = min(source2.data['close']) * 0.3
        p2.y_range.end = max(source2.data['close']) * 1.05
    ...
    def update_stat(symbol1, symbol2):
        ...
        des[symbol1] = df_all[df_all.symbol == symbol1]['close']
        des[symbol2] = df_all[df_all.symbol == symbol2]['close']
    ...
    # create plot
    # hover setting
    p2 = figure(title='{}  (Click on legend entries to hide the corresponding lines)'.format(symbol_list[1]),
                plot_height=300,
                tools="crosshair,save,undo,xpan,xwheel_zoom,ybox_zoom,reset",
                active_scroll='xwheel_zoom',
                x_axis_type="datetime",
    ...
    p2.line('time', 'ma5', line_width=1, color=palte[5], alpha=0.8, muted_color='navy', muted_alpha=0.1, legend='MA5',
            source=source2)
    p2.line('time', 'ma10', line_width=1, color=palte[6], alpha=0.8, muted_color='navy', muted_alpha=0.1, legend='MA10',
            source=source2)
    p2.line('time', 'ma30', line_width=1, color=palte[7], alpha=0.8, muted_color='navy', muted_alpha=0.1, legend='MA30',
            source=source2)

    # p2.y_range = Range1d(min(source2.data['adjusted_close']) * 0.3, max(source2.data['adjusted_close']) * 1.05)
    p2.y_range = Range1d(min(source2.data['close']) * 0.3, max(source2.data['close']) * 1.05)
    ...
    p2.vbar('time', width=3, top='volume', color=choice(all_palettes['Set2'][8]), alpha=0.5, y_range_name="volumes",
            source=source2)
