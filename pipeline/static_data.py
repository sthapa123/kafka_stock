...
def get_yahoo():
    modules = [
        ...
        'quoteType',
        'recommendationTrend', 'secFilings', 'sectorTrend', 'summaryDetail', 'summaryProfile', 'symbol',
    ...
    for symbol in symbol_list:
        if symbol != '^GSPC':
            url = "https://query2.finance.yahoo.com/v10/finance/quoteSummary/{" \
                  "}?formatted=true&lang=en-US&region=US&modules={}&corsDomain=finance.yahoo.com".format(
                symbol, '%2C'.join(modules))
...
def get_economy():
    ...
    keys = ['GDPC1', 'CPIAUCSL', 'INDPRO', 'DGS10', 'UNRATE', 'PAYEMS', 'FEDFUNDS', 'DTB3', 'DGS5', 'DGS10', 'DGS30',
            'GFDEBTN']
...
def get_bea():
    ...
    for tablename in RegionalTable:
        url = 'https://apps.bea.gov/api/data/?UserID={}&method=GetData&datasetname=Regional&LineCode=1&TableName={}&GeoFips=STATE&Year=LAST5&ResultFormat=json'.format(
            bea_api, tablename)
        req = requests.get(url)
        print(tablename)
        csv(req)
